    ...
    <div class="content">
      <div class="search-box">
        ...
        <div>
          ...
          <a-button
            ...
            :disabled="btnLoading"
            v-show="!showBtn"
            >{{ btnLoading ? "成果获取中..." : "成果获取" }}</a-button
          >
          <a-button type="primary" @click="handleDownload" v-show="showBtn"
            >成果下载</a-button
          >
        </div>
      </div>
      ...
      >
        <template #final_score="{ record, text }">
          <span
            :class="{ linkText: text !== '--' }"
            @click="checkDetail('score', record, text !== '--')"
            >{{ text }}</span
          >
        </template>
        <template #show="{ record }">
          <span
            :class="['text', { linkText: record.has_note }]"
            @click="checkDetail('notes', record.note_id, record.has_note)"
            >笔记</span
          >
          <span
            :class="[
              ...
              {
                linkText:
                  record.has_question_record || record.has_rebuild_question_record,
              },
            ]"
            @click="
              checkDetail(
          ...
          >
          <span
            :class="['text', { linkText: record.has_video }]"
            @click="checkDetail('video', record.video_ids, record.has_video)"
            >视频</span
          >
        </template>
        <template #has_report="{ record }">
          <span
            :class="['text', { linkText: record.report_id }]"
            @click="checkDetail('report', record.report_id, record.report_id)"
            >{{
              !record.has_report
                ? "未提交"
                : record.is_report_scored && record.report_score
                ? record.report_score
                : "批阅"
            }}</span
          >
        ...
        <template #has_rebuild_report="{ record }">
          <span
            :class="['text', { linkText: record.report_rebuild_id }]"
            @click="
              checkDetail('report', record.report_rebuild_id, record.report_rebuild_id)
            "
            >{{
              !record.has_rebuild_report
                ? "未提交"
                : record.is_rebuildreport_scored && record.rebuild_report_score
                ? record.rebuild_report_score
                : "批阅"
            }}</span
          >
        </template>
        <template #can_evaluate="{ record }">
          <div v-if="role === '2'">
            <span class="text" v-if="!record.evaluated">未评价</span>
          ...
          <div v-else>
            <span :class="['text', { resultText: record.can_evaluate }]">
              <span
                class="evaluteText"
                v-if="!record.evaluated"
                @click="checkDetail('evalute', record, record.can_evaluate)"
                >评价</span
              >
      ...
      http.resultCreate({ param: params }).then((res: IBusinessResp) => {
        if (res && res.data) {
          ...
          let timer: any = setInterval(() => {
            http
              .exportTaskView({ param: { id: resultId.value } })
              .then((result: IBusinessResp) => {
