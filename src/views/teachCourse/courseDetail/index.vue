<template>
  <div v-layout-bg="{ size: '234px', bg: bg }" style="height:calc(100% - 24px);">
    <top></top>
      <a-tabs default-active-key="1" @change="tabChange" class="course-detail-tabs">
        <a-tab-pane key="1" tab="课程实验" v-if="tabs.includes('courseExperimentTab')"> 
          <course-experiment-tab></course-experiment-tab>
        </a-tab-pane>
        <a-tab-pane key="2" tab="课程成员" force-render v-if="tabs.includes('courseMembersTab')">
          <course-members-tab></course-members-tab>
        </a-tab-pane>
        <a-tab-pane key="3" tab="分组信息" force-render v-if="tabs.includes('groupInfoTab')"> 
          <group-info-tab></group-info-tab>  
        </a-tab-pane>
        <a-tab-pane key="4" tab="课程资源" force-render v-if="tabs.includes('courseResourcesTab')"> 
    ...
    function settingTab() {
      if (![1, 3].includes(currentRole)) {
        ...
        if (currentRole == 3 && currentTab === "myCourse") {
          return ['courseExperimentTab','courseResourcesTab','groupInfoTab','courseMembersTab']
        }
        return [];
      }
    }
