<template>
  <div class="paper-content-list" v-for="(list, index) in modelValue" :key="list.id">
    ...
    <!-- 单选题 -->
    <a-radio-group class="radioclass" v-model:value="list.answers[0].answer" v-if="list.type_id === 1">
      <ul>
        <li v-for="(item, i) in list.options" :key="i">
          <a-radio disabled :value="item.id" />
          <span>{{options[i]}}：{{item.option}}</span>
          <span class="answer" v-if="Number(list.answers[0].answer) === item.id">(正确答案)</span>
        </li>
      </ul>
    </a-radio-group>
    <!-- 多选题 -->
    <a-checkbox-group class="radioclass" v-model:value="list.answersList" v-if="list.type_id === 2">
      <ul>
        <li v-for="(item, i) in list.options" :key="i">
          <a-checkbox disabled :value="item.id" />
          <span>{{options[i]}}：{{item.option}}</span>
          <span class="answer" v-if="list.answersList.includes(item.id)">(正确答案)</span>
        </li>
    </ul>
    </a-checkbox-group>
    <!-- 判断 -->
    ...
    <div v-if="list.type_id === 5">
      <span>答案</span>
      <span class="answer">{{list.keywordsList}}</span>
    </div>
    <div class="icons">
      <span class="iconfont icon-yichu" @click="del(index)"></span>
