  ...
  <a-config-provider :renderEmpty="customizeRenderEmpty">
    ...
    >
      ...
      <template #operation="{ record }">
        ...
        <a class="caozuo" @click="editCard(record)" v-if="tabType === 1"
          >编辑</a
        >
        <a class="caozuo" @click="delateCard(record)" v-if="record.can_delete"
          >删除</a
        >
      </template>
    </a-table>
  ...
  >
    ...
    <div class="text">
      <QuillEditor
        toolbar=""
        :height="'300px'"
        v-model="ForumArticle.content"
        :uploadPathName="'studentForum'"
      />
    </div>
    <template #footer>
...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    const tabs = [
      { name: "随堂论坛", componenttype: 0 },
      { name: "我的提问", componenttype: 1 },
      { name: "我参与的帖子", componenttype: 2 },
    ];
    ...
    const customizeRenderEmpty = function (): VNode {
      if (loading.value) {
        ...
        let type =
          ForumSearch.title || ForumSearch.type !== undefined
            ? "tableSearchEmpty"
            : "tableEmpty";
    ...
    function initData() {
      ...
      loading.value = true;
      list.length = 0;
      http[apiName[tabType.value]]({ param: { ...ForumSearch } }).then(
        (res: IBusinessResp) => {
          loading.value = false;
    ...
    function handleReply() {
      ...
      http
        .handleReply({ param: { ForumArticle: { ...obj } } })
        .then((res: IBusinessResp) => {
          if (res.status === 1) {
            initData();
