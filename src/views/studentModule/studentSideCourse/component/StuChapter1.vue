    ...
    <div class="main" v-if="chapterData.nums">
      <div v-if="chapterData.status === 0" class="mainrightTop">
        <div>
          ...
          <span class="tip"
            >(共{{ chapterData.nums }}题,满分{{ totalScoreAll }}分)</span
          >
        </div>
        <div>
      ...
      <div class="chapter setScrollbar">
        <div
          class="chapterList"
          :style="
            chapterData.status === 0
              ? 'pointer-events:auto'
              : 'pointer-events:none'
          "
          v-for="item in chapterData.quest_list_questions"
          :key="item.id"
        >
          <single-echoice
            v-if="item.type === 1"
...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    var totalScoreAll = computed(() => {
      ...
      if (
        chapterData.quest_list_questions &&
        chapterData.quest_list_questions.length
      ) {
    ...
    function init() {
      ...
      http
        .questionsList({
          param: { course_id: course_id, chapter_id: chapter_id.value },
        })
        .then((res: IBusinessResp) => {
          if (res) {
            let data = res.data;
            data.quest_list_questions
              ? data.quest_list_questions.map((v: any, k: Number) => {
                  v.index = k;
                })
              : "";
