<template>
  <div class="item custom_select">
    <a-select
      v-model:value="course_category_id"
      placeholder="请选择课程方向"
      :options="options"
      @change="handleChange"
    ></a-select>
        ...
        >
          <div class="main">
            ...
            <div class="start_training">
              <a-button @click="keepLearning(v)" type="link">
                {{ v.used_time !== "0秒" ? "继续" : "开始" }}学习</a-button
              >
            </div>
          </div>
        </div>
...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    var options = ref<Ioptions[]>([
      { id: 1, name: "全部", value: 1, label: "全部课程" },
    ]);
    ...
    var defaultUrl: string =
      "/src/assets/images/studentcourse/course-default1.jpg";
    function initData() {
      let param =
        course_category_id.value !== 1
          ? { course_category: course_category_id.value }
          : {};
      loading.value = true;
      list.length = 0;
    ...
    function keepLearning(val: IListItem) {
      ...
      router.push(
        "/studentSideCourse/ContinueDetail?DetailId=" +
          val.course_student_id +
          "&course_id=" +
          val.cousre_id +
          "&currentTab=" +
          currentTab
