    ...
    <div class="header" v-layout-bg>
      <div class="search">
        ...
        <div  class="item custom_input custom_input3">
          <a-input v-model:value="ForumSearch.teacher" placeholder="请输入所属教师" @keyup.enter="search()" />
        </div>
        <div class="item">
...
export default defineComponent({
  ...
  setup: (props,{emit}) => {
    ...
    function initData(){
      ...
      loading.value=true
      list.length=0
      http.teacherManageList({param:{...ForumSearch,withs:'userProfile'}}).then((res:IBusinessResp)=>{
        if(res){
          loading.value=false
