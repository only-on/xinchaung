    ...
    <div class="header" v-layout-bg>
      <div class="search">
        <div class="item custom_input custom_input1">
          <a-input v-model:value="ForumSearch.name" placeholder="请输入技术方向名称" @keyup.enter="search()" />
        </div>
        <div class="item">
    ...
    <a-config-provider :renderEmpty="customizeRenderEmpty">
      <a-table :columns="columns" :loading="loading" :data-source="list" :bordered="true"  row-key="id"
        ...
        <template #operation="{record}">
          <i class="caozuo iconfont icon-chakan1" @click="details(record.id )" title="查看"></i>
        </template>
      </a-table>
    </a-config-provider>
</template>
...
export default defineComponent({
  ...
  setup: (props,{emit}) => {
    ...
    function initData(){
      ...
      loading.value=true
      list.length=0
      http.TechnicalDirection({param:{...ForumSearch,withs:'userProfile'}}).then((res:IBusinessResp)=>{
        if(res){
          loading.value=false
