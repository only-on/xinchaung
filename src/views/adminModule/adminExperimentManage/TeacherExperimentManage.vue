  ...
  <div class="header" v-layout-bg>
    <div class="search">
      ...
      <div class="item custom_input custom_input2">
        <a-input
          v-model:value="ForumSearch.category"
          placeholder="请输入所属技术方向"
          @keyup.enter="search()"
        />
      </div>
      <div class="item custom_input custom_input3">
        <a-input
          v-model:value="ForumSearch.teacher"
          placeholder="请输入所属教师"
          @keyup.enter="search()"
        />
      </div>
  ...
  setup: (props, { emit }) => {
    ...
    const customizeRenderEmpty = function (): VNode {
      if (loading.value) {
        ...
        let type =
          ForumSearch.name || ForumSearch.name
            ? "tableSearchEmpty"
            : "tableEmpty";
    ...
    function initData() {
      ...
      list.length = 0;
      http
        .teacherManageList({ param: { ...ForumSearch, withs: "userProfile" } })
