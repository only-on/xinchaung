  ...
  <div class="header" v-layout-bg>
    <div class="search">
      ...
      <div class="item custom_input custom_input2">
        <a-input
          v-model:value="ForumSearch.students_count"
          placeholder="请输入学生人数"
          @keyup.enter="search()"
        />
      </div>
  ...
  setup: (props, { emit }) => {
    ...
    const customizeRenderEmpty = function (): VNode {
      if (loading.value) {
        ...
        let type =
          ForumSearch.students_count || ForumSearch.name
            ? "tableSearchEmpty"
            : "tableEmpty";
    ...
    function initData() {
      ...
      ForumSearch.students_count
        ? (param["search[students_count]"] = ForumSearch.students_count)
        : "";
    ...
    function delateCard(val: number) {
      ...
      Modal.confirm({
        ...
        onOk() {
          http
            .classUserDelete({ urlParams: { class_id: val } })
    ...
    function BatchDelete() {
      ...
      Modal.confirm({
        ...
        onOk() {
          http
            .classUserBatchDelete({
              param: { class_ids: state.selectedRowKeys },
            })
            .then((res: IBusinessResp) => {
              initData();
              message.success("删除成功");
    ...
    function editCard(val: ItdItems) {
      ...
      // visible.value=true
      router.push(
        "/admin/adminUserManagement/classManagement/classEdit?editId=" + val.id
      );
    }
