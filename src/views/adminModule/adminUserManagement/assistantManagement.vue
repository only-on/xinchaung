<template>
  <div class="header" v-layout-bg>
    ...
    <div class="addTeacher">
      <a-button @click="BatchDisable()" type="primary" v-if="!showAdd"
        >批量禁用</a-button
      >
      <a-button @click="addTeacher()" type="primary" v-if="showAdd"
        >添加助教</a-button
      >
    </div>
    ...
    >
      <template #status="{ record }">
        <a-switch
          ...
          :disabled="showAdd === true ? false : true"
        />
      </template>
  ...
  setup: (props, { emit }) => {
    ...
    function delateCard(val: number) {
      ...
      Modal.confirm({
        ...
        onOk() {
          http
            .AssistantDelete({ urlParams: { id: val } })
    ...
    function BatchDisable() {
      ...
      http
        .AssistantBatchDisable({ param: { user_ids: state.selectedRowKeys } })
        .then((res: IBusinessResp) => {
          initData();
          state.selectedRowKeys.length = 0;
          message.success("操作成功");
