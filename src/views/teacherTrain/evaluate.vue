<template>
  <div class="evaluate setScrollbar" v-layout-bg>
    ...
    <div class="condition">
      <a-form-item>
        <a-input @keyup.enter="query" placeholder="学号" v-model:value="searchInfo.name">
          <template #prefix>
            <img src="src/assets/images/screenicon/Group7.png" /> </template
        ></a-input>
      </a-form-item>
      <a-form-item>
        <a-input @keyup.enter="query" placeholder="姓名" v-model:value="searchInfo.nick"
          ><template #prefix>
            <img src="src/assets/images/screenicon/Group6.png" /> </template
        ></a-input>
      </a-form-item>
      <a-form-item>
        <a-input @keyup.enter="query" placeholder="班级" v-model:value="searchInfo.class">
          <template #prefix>
      ...
      <div class="right">
        <a-button type="primary" class="upload" @click="exportResult()"
          >导出成绩</a-button
        >
        <a-button type="primary" class="upload" @click="releaseResult()"
          >发布成绩</a-button
        >
      </div>
    </div>
  ...
  setup() {
    ...
    const exportScore = (param: { train_id: string; id?: number[] }) => {
      ...
      fetch(url, {
        ...
        body: param.id
          ? `train_id = ${param.train_id} & train_student_id = ${param.id}`
