  ...
  <div class="trainEnvironment">
    <div class="top-info-btn">
      <div>
        ...
        <span>实训环境</span>
        <span class="orange"
          >已选总内存：<span class="number">{{ memoryNumber() }}</span
          >G</span
        >
      </div>
      <div v-if="role !== '2'">
    ...
    <div class="listTable">
      <div
        class="listItem"
        v-for="(item, index) in propTrainDetailInfo.container"
        :key="index.toString()"
      >
        ...
        <div class="itemselect">
          <a-form-item label="cpu">
            <a-select
              :disabled="!edit"
              default-value="1"
              v-model:value="item.cpu"
              class="selectWidth"
            >
              <a-select-option
                v-for="(item, index) in selectOption?.cpu"
                :key="index.toString()"
                :value="item.key"
              >
                {{ item }}核
          ...
          <a-form-item label="内存">
            <a-select
              :disabled="!edit"
              default-value="2048"
              v-model:value="item.ram"
              class="selectWidth"
            >
              <a-select-option
                v-for="(item, index) in selectOption?.ram"
                :key="index.toString()"
                :value="item.key"
              >
                {{ item }}
          ...
          <a-form-item label="硬盘">
            <a-select
              :disabled="!edit"
              default-value="30"
              v-model:value="item.disk"
              class="selectWidth"
            >
              <a-select-option
                v-for="(item, index) in selectOption?.disk"
                :key="index.toString()"
                :value="item.key"
              >
                {{ item }}
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    watch(
      () => props.propTrainDetailInfo.container,
      (val: any) => {
