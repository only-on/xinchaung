  ...
  <div v-layout-bg class="detail">
    <div class="head-exam fixCenter">
      ...
      <div class="right">
        <div class="examNum">
          <div class="">
            试卷总分：<span>{{ totalScore }}</span>
          </div>
          <div class="">
            已选试题：<span>{{
              selectedPaperIds && selectedPaperIds.length
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div class="questions">
      <div class="title">
        <div
          class="tab"
          :class="activePaper === v.type ? 'activePaper' : ''"
          v-for="v in PaperList"
          :key="v.type"
          @click="activePaper = v.type"
        >
          ...
          <div class="PaperType setScrollbar">
            <div class="item" v-for="(v, k) in activeData" :key="v">
              ...
              <template v-if="activePaper === 'duo' || activePaper === 'dan'">
                <div
                  class="options"
                  v-for="(i, k) in v.options"
                  :key="i"
                  :class="answers(i.id, v.answers) ? 'selected' : ''"
                >
                  {{ `选项${option[k]}、${i.option}` }}
                </div>
              </template>
              ...
              <template v-if="activePaper === 'tian' || activePaper === 'jian'">
                <div class="options">
                  {{ v.answers[0] && v.answers[0].answer }}
                </div>
              </template>
            </div>
          </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    var PaperList: IPaperList[] = reactive([
      {
        name: "单选题",
        type: "dan",
    ...
    function getPaperDetail(val: number) {
      http
        .getPaperDetail({ urlParams: { paper_id: val } })
    ...
    function getDetailQuestions(val: number) {
      http
        .getDetailQuestions({ urlParams: { paper_id: val } })
        .then((res: IBusinessResp) => {
          // console.log(res)
          let data = res.data;
          data.forEach((v: any) => {
