<template>
  <div class="createProgress1" v-layout-bg>
    <a-form ref="formRef" layout="vertical" :model="formState" :rules="rules">
      <div class="main">
        <div class="left">
          ...
          <div>
            <div>
              <span class="cover_word">实训封面</span
              ><span class="type_title"
                >支持格式：ipg、png,限制尺寸525*300px</span
              >
            </div>
            <upload-image
              :uploadUrl="formState.url"
              @img-src="imgSrc"
              createOrEdit="create"
            ></upload-image>
          </div>
    ...
    <div class="foot">
      ...
      <a-button class="next" type="primary" @click.prevent="onSubmit">
        下一步
      </a-button>
    </div>
  </div>
    ...
    const methods = {
      ...
      onSubmit() {
        ...
        if (state.formState.name)
          if (
            Number(state.formState.train_time) < 1 ||
            Number(state.formState.train_time) > 16 ||
            Number(state.formState.train_time) % 1 !== 0
          ) {
