 ...
 * @description 保存镜像
 */
export async function createImageApi(
  params: { name: string; description: string },
  urlParams: { id: number }
) {
  return await workbenchApi.createImageApi({
    urlParams: urlParams,
    param: params,
  });
}
 ...
 * @description 虚拟机文件上传
 */
export async function vmUploadApi(
  params: { file_path: string },
  urlParams: { id: number }
) {
  return await workbenchApi.vmUploadApi({
    urlParams: urlParams,
    param: params,
  });
}
