<template>
  ...
  <div class="teacherAssignmentEditFooter">
    <div class="flexCenter"> 
      <Submit :okText="'保存作业'" @submit="save" @cancel="cancel" :loading="loading"></Submit>
    </div>
  </div>
  <!-- 编辑基本信息 -->
  <EditBaseInfo v-model:visible="editModal" :id="id" :type="'作业'" :isEdit="true" @updateInfo="getExamDetail"/>
</template>
...
const cancel=()=>{
  router.go(-1)
  // router.replace({
  //   path:'/teacher/teacherAssignment/teacherAssignmentPreview',
  //   query:{id:id}
  // })
}
const save=()=>{
