<template>
  ...
  >
    <div class="nav-list">
      <span
        v-for="v in typeList.paperType"
        :key="v.id"
        :class="{ 'current-nav': selectType === v.id }"
    ...
    <div class="list-content">
      <div class="lists">
        <div class="search">
          <a-select
            v-model:value="searchInfo.poolId"
            placeholder="请选择题目目录"
            @select="changeDirectory"
          >
            <a-select-opt-group label="公有">
              <a-select-option
                :value="item.id"
                v-for="item in directoryList.commonList"
                :key="item.id"
            ...
            <a-select-opt-group label="私有">
              <a-select-option
                :value="item.id"
                v-for="item in directoryList.privateList"
                :key="item.id"
                >{{ item.name }}</a-select-option
              >
            </a-select-opt-group>
          ...
          <a-select
            v-model:value="searchInfo.levelId"
            placeholder="请选择试题难度"
            class="difficulty"
            @click="changeLevel"
          >
            <a-select-option v-for="v in typeList.levelType" :value="v.id" :key="v.id">{{
              v.name
            }}</a-select-option>
          </a-select>
          <a-input-search
            ...
            @search="onSearch"
          />
          <a-button type="primary" @click="selectAllHandle"
            >全选（{{ paperList.length }}）</a-button
          >
        </div>
        <ul v-if="paperList.length">
          <li v-for="(list, i) in paperList" :key="i">
            <p>
              {{ list.question }}<span>（{{ list.origin_score }}）</span>
            </p>
            <span class="iconfont icon-del" @click="add(i)" v-if="!list.isChecked"></span>
            <span
              class="iconfont icon-yichu"
              @click="del(i)"
              v-if="list.isChecked"
            ></span>
          </li>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const selectAllHandle = () => {
      ...
      list.paperList.forEach((list) => {
        ...
        if (list.answers.length) {
          props.selectType === 1 || props.selectType === 2 || props.selectType === 3
            ? list.answers.forEach((v: any) => {
    ...
    const del = (i: number) => {
      ...
      emit(
        "update:modelValue",
        arr?.filter((v) => v.id !== list.paperList[i].id)
      );
    };
    ...
    let directoryList: IDirectoryList = inject("directoryList") || {
      commonList: [],
      privateList: [],
    };
    ...
    watch(
      () => directoryList,
      (newVal) => {
        searchInfo.poolId = newVal.commonList.length
          ? newVal.commonList[0].id
          : newVal.privateList[0].id;
        // getQuestionList()
      },
      { deep: true }
    );
