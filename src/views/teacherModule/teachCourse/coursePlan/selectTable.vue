      ...
      <div class="list-wrapper scroll-view-wrapper">
        ...
        >
          <template #action="{ record }">
            <span
              class="iconfont icon-yichu1"
              @click="handleDelSelectData(record.stu_id)"
              v-if="!neverShow(record)"
            ></span>
            <span
              class="iconfont icon--tainjia"
              @click="handleSelectData(record)"
              v-if="neverShow(record)"
            ></span>
          </template>
        </a-table>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    var onDeleteStudent = inject("onDeleteStudent") as Function;
    function handleDelSelectData(stu_id: number) {
      onDeleteStudent(stu_id);
    }
