  ...
  >
    <a-tree-node v-for="item in treeData" :key="item.id">
      <template v-slot:title>
        <div :class="selectid === item.id ? 'tree-first-title selectbac' : 'tree-first-title'" @click="selectChapter(item)">
          <div>
            <span class="icon-zhangjie iconfont chapter-cion"></span>
            <span class="chapter-name">{{ item.name }}</span>
            <span class="action-icon-box">
              <span v-if="isOperation" @click.stop="editNode(item)" class="icon-bianji1 iconfont"></span>
              <span v-if="isOperation" @click.stop="deleteNode(item)" class="icon-shanchu iconfont"></span>
      ...
      <a-tree-node v-for="ct in item.contents" :key="ct.id">
        <template v-slot:title>
          ...
          <div :class="selectid === ct.id ? 'tree-second-title selectbac' : 'tree-second-title'" @click="selectExperiment(ct)">
            <div :class="ct.is_high ? 'height-experiment' : ''">
              <span class="experiment-type">
                <span class="iconfont type-icon" :class="ct.is_high ? 'icon-jinzhi' : getExperimentClassName(ct.type, 'icon')"></span>
...
<script lang="ts">
import { defineComponent, onMounted, PropType, reactive, ref, toRefs, createVNode, computed, watch } from 'vue';
    ...
    function onDrop(info: DropEvent) {
      ...
      if (!info.dropToGap) {
        ...
        if (dragKeyParentId === dropKeyParentId) {
          loop(data, dragKey, (item: ITreeDataItem, index: number, arr: ITreeDataItem[]) => {
            arr.splice(index, 1)
            dragObj = item
          })
          let ar: ITreeDataItem[] = []
          let i: number = 0
          loop(data, dropKey, (item: ITreeDataItem, index: number, arr: ITreeDataItem[]) => {
