<template>
  <div class="image-box">
    <div class="tags">
      ...
      <span :class="currentType === key ? 'active' : ''" v-for="(item, key) in imageTypes" :key="key" @click="tabChange(key)">
        {{item}}
      </span>
    </div>
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    function del(i: number) {
      ...
      emit('update:modelValue', arr?.filter((v: IimageData) => v.id !== imageData[i].id))
    }
    // 获取配置
...
.image-box {
  ...
  .image-list {
    ...
    .image-item {
      ...
      &:hover {
        background: rgba(var(--primary-1),.2);
        border: 1px solid rgba(var(--primary-color),.7);
        border-radius: 6px;
